    ...
    switch (provider) {
        case Provider.TWITTER:
            if (!TWITTER_API_KEY || !TWITTER_API_SECRET || !TWITTER_REDIRECT_URI) {
                return NextResponse.json({ error: "Brak konfiguracji Twitter" }, { status: 500 });
            }
            ...
            if(!requestTokenResponse.ok) {
                ...
                return NextResponse.json({ error: "Nie udało się pobrać tokena.", details: errorTxt || undefined}, { status: 500 });
            }
            ...
            if(!requestToken || !requestTokenSecret) {
                return NextResponse.json({ error: "Brak wymaganych tokenów w odpowiedzi."}, { status: 500 });
            }

            if(requestParams.get("oauth_callback_confirmed") !== "true") {
                return NextResponse.json({ error: "Twitter nie potwierdził poprawnego callbacku."}, { status: 500 });
            }
            ...
            return NextResponse.json({ authUrl: "https://api.x.com/oauth/authorize?oauth_token=" + requestToken });
        default:
            return NextResponse.json({error: "Nieobsługiwany provider"}, { status: 400 })
    }
