  ...
  try {
    ...
    if (s3MediaUrls.length > 0) {
      for (const media of s3MediaUrls) {
        const creationUrl = `https://graph.facebook.com/v23.0/${instagramUserId}/media`;
    ...
    const publishUrl = `https://graph.facebook.com/v23.0/${instagramUserId}/media_publish`;
    ...
    if (mediaObjectIds.length === 1) {
      ...
      // Karuzela (album)
      const carouselUrl = `https://graph.facebook.com/v23.0/${instagramUserId}/media`;
